...
function handleLogout() { localStorage.removeItem('edustream_session'); window.location.href = '/portal'; }
function checkAuthStatus() {
    ...
    if (s) {
        ...
        if (cur === 'portal') window.location.href = `${r}-dashboard.html`; else if (cur.includes('-dashboard.html') && !cur.startsWith(r)) window.location.href = `${r}-dashboard.html`;
        if (cur === 'lecturer-dashboard.html') { renderAnnouncements(); renderAssignments(); } if (cur === 'admin-dashboard.html') { renderLogs(); renderUsers(); }
    } else {
        auth.forEach(i => i.style.display = 'none'); if (nBtn) { nBtn.innerText = 'Sign In'; nBtn.className = 'btn btn-primary'; nBtn.onclick = null; nBtn.href = '/portal'; }
        if (cur.includes('-dashboard.html') || cur === 'forum.html') window.location.href = '/portal';
    }
}
...
// --------------------------------------------------
// Student Portal: Login & Registration[cite: 1]
// --------------------------------------------------
async function handleLogin(username, password) {
    ...
    if(response.ok) {
        ...
        localStorage.setItem('currentUser', data.username); // Save session
...
// --------------------------------------------------
// Course Content Pages[cite: 1]
// --------------------------------------------------
async function loadCourses() {
    const response = await fetch('/api/courses');
    const courses = await response.json();
    const container = document.getElementById('course-list'); // Ensure this ID exists in courses.html
    if (container) {
        container.innerHTML = courses.map(c => 
            `<div class="course-card">
                <h3>${c.title}</h3>
                <p>Module: ${c.module} | Type: ${c.type}</p>
            </div>`
        ).join('');
    }
}
...
// --------------------------------------------------
// Discussion Forum[cite: 1]
// --------------------------------------------------
async function loadForumPosts() {
    const response = await fetch('/api/forum');
    const posts = await response.json();
    const container = document.getElementById('forum-container'); // Ensure this ID exists in forum.html
    if (container) {
        container.innerHTML = posts.map(p => 
            `<div class="post">
                <strong>${p.author}</strong> <span>${new Date(p.time).toLocaleString()}</span>
                <p>${p.content}</p>
            </div>`
        ).join('');
    }
...
async function submitPost(content) {
    const author = localStorage.getItem('currentUser') || 'Anonymous Student';
    await fetch('/api/forum', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ author, content })
    });
    loadForumPosts(); // Reload posts after submitting
}

// --------------------------------------------------
// Bookstore Module & Checkout[cite: 1]
// --------------------------------------------------
let shoppingCart = [];
...
async function loadBooks() {
    ...
    const container = document.getElementById('book-container'); // Ensure this ID exists in bookstore.html
    if (container) {
        container.innerHTML = books.map(b => 
            `<div class="book-card">
                <h4>${b.title}</h4>
                <p>${b.category} | RM ${b.price}</p>
...
function addToCart(bookId) {
    shoppingCart.push(bookId);
    alert(`Book added! Cart total items: ${shoppingCart.length}`);
}
...
async function checkoutCart() {
    ...
    const data = await response.json();
    if(response.ok) {
        alert(`Success! Total Paid: RM ${data.total_paid}`);
        shoppingCart = []; // Clear cart
        loadBooks(); // Refresh inventory numbers
    } else {
...
// Initialization
document.addEventListener('DOMContentLoaded', () => {
    if(window.location.pathname === '/courses') loadCourses();
    if(window.location.pathname === '/forum') loadForumPosts();
    if(window.location.pathname === '/bookstore') loadBooks();
});
